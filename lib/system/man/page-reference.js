...
class ManPageReference{
	 ...
	 *
	 * @return {String[]}
	 *    A page instance populated with group number and
	 *    suffix, if one was provided in the arguments.
	 */
	static resolve(...args){
		switch(args.length){
			case 0:
			...
			case 1:
				return /^\s*([^()<>\s]+)\s*\(([^)\s]+)\)\s*$/.test(args[0])
					? new ManPageReference({name: RegExp.$1, section: RegExp.$2})
					: new ManPageReference({name: args[0], section: ""});
			default:
