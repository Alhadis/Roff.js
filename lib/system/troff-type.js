...
class TroffType{
	...
	async guessPipeline(source, format = "tty"){
		...
		if(this.hasGrog && await this.shell.which("grog")){
			const {stdout} = (await this.shell.exec("grog", ["-ZTutf8"], source));
	...
	static async resolveDefault(ignoreCache = false){
		...
		// GNU Troff
		let path = await this.shell.which("groff");
		if(path){
		...
		// "Traditional" Troff
		if(path = await this.shell.which("troff")){
			
			// Use feature-level detection provided by `which.roff'
			const which  = await this.shell.loadFile(join(__dirname, "which.roff"));
			const result = await this.shell.exec("troff", [], which);
			...
			switch(process.platform){
				...
				case "sunos":
					const version = parseFloat(await this.shell.exec("uname", ["-v"]));
		...
		// OpenBSD/mandoc
		if(path = await this.shell.which("mandoc")){
			const version = parseFloat((await this.shell.exec("uname", ["-r"])).stdout);
	...
	static async resolveGroffVersion(){
		const {stdout} = await this.shell.exec("groff", ["--version"]);
