...
const shell = require("./shell-cache.js").sharedInstance;
...
class TroffType{
	 ...
	 * To force re-evaluation, set `ignoreCache` to a truthy value.
	 *
	...
	static async resolveDefault(ignoreCache = false){
		...
		if(path){
			...
			// Extract version string from `groff --version` output.
			const {stdout} = await shell.exec("groff", ["--version"]);
			if(/^GNU\s+groff\s+version\s+v?([0-9]+\.[0-9]+\.[0-9]+[^0-9\s]*)/i.test(stdout))
				version = RegExp.$1;
			
			return this.defaultTroff = new TroffType({path, name: this.GROFF, version});
		}
		...
		if(path = await shell.which("troff")){
			...
			switch(result && result.stderr.trim()){
				...
				// Heirloom Doctools
				case this.HEIRLOOM:
					return this.defaultTroff = new TroffType({path, name: this.HEIRLOOM});
				
				// NOTE: This shouldn't happen. We include this to improve the chances of being
				//       compatible with future re-implementations of troff(1). Groff parity is
				//       precariously assumed based on support for `\n[long-name]' syntax.
				case this.GROFF:
					return this.defaultTroff = new TroffType({path, name: this.GROFF_LIKE});
				
				// This shouldn't happen either, but it's too easy to check
				case this.MANDOC:
					return this.defaultTroff = new TroffType({path, name: this.MANDOC});
			}
			
			// DWB 3.3 installs troff to `$(PREFIX)/dwb/bin/troff' by default
			if(/([/\\])dwb\1bin\1troff$/i.test(path))
				return this.defaultTroff = new TroffType({path, name: this.DWB});
			
			// Gunnar Ritter's port of Solaris 10's troff (untested)
			if(/([/\\])sunroff\1bin\1/i.test(path))
				return this.defaultTroff = new TroffType({path, name: this.SOLARIS10});
			
			// Gunnar Ritter's port of 9front troff
			if(/([/\\])9froff\1bin\1/i.test(path))
				return this.defaultTroff = new TroffType({path, name: this.FRONT});
			
			// Last resort: blindly guess platform-specific troff
			switch(process.platform){
				// IBM AIX/IRIX
				case "aix":
					return this.defaultTroff = new TroffType({path, name: this.AIX});
				...
				case "sunos":
					...
					if(version >= 11 && version < 12)
						return this.defaultTroff = TroffType({path, version, name: this.SOLARIS11});
					
					// Solaris 10 (untested)
					if(version >= 10 && version < 11)
						return this.defaultTroff = TroffType({path, version, name: this.SOLARIS10});
					
					break;
			}
			
			// No idea. Cross our fingers and hope this program quacks like troff
			return this.defaultTroff = new TroffType({path, name: this.UNKNOWN});
		}

		// OpenBSD/mandoc
		if(path = await shell.which("mandoc")){
			const version = parseFloat((await shell.exec("uname", ["-r"])).stdout);
			return this.defaultTroff = new TroffType({path, version, name: this.MANDOC});
		}
