...
/**
 * Immutable reference to a manpage.
 * @class
 */
class PageReference{
	 ...
	 *
	 * Once constructed, the instance's properties
	 * are frozen to ensure immutability.
	 *
	 * @param {Object} [attr={}]
	 * @constructor
	 */
	 ...
	 *
	 * @see {PageReference.parseURL}
	 * @example grep.toURL() === "man://man/1";
	 * @param {Boolean} [useCGI] - Format URL for man.cgi(8)
	 * @return {String}
	 */
	toURL(){
		const path = [
	...
	/**
	 * Reference to the global/shared instance map.
	 * @property {Map}
	 * @readonly
	 */
	...
	/**
	 * Create a new manpage reference, or reuse an existing one.
	 *
	 * Instances are stored and accessed via {@link PageReference.instances}.
	 *
	 * @public
	 * @example PageReference.get("grep", "1", "sparc64");
	 * @param {String} name
	 * @param {String} [section="1"]
	 * @param {String} [arch=""]
	 * @param {String} [manPath=""]
	 * @return {PageReference}
	 */
	static get(name, section = "1", arch = "", manPath = ""){
		const attr  = {name, section, arch, manPath};
		let pageURI = this.prototype.toURL.call(attr);
		let pageRef = instances.get(pageURI);
		if(!pageRef){
			pageRef = new PageReference(attr);
			instances.set(pageURI, pageRef);
		}
		return pageRef;
	}
	 ...
	 * @public
	 * @example <caption>Basic usage</caption>
	 *    PageReference.fromURL("man://roff/7#history") => {
	 *       name: "roff",
	 *       section: "7",
	 *       fragment: "history",
	 *    };
	 ...
	 *    PageReference.fromURL("man://sparc64/lom.4", true) => {
	 *       name: "lom",
	 *       section: "4",
	 *       arch: "sparc64",
	 *    };
