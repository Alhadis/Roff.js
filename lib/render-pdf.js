						...
						switch(args[0].toLowerCase()){
							...
							case "pdf:":
								switch(args[1]){
									// Begin hotspot
									case "markstart":{
										const meta   = args.slice(5).join(" ");
										const rst    = (+args[2] + +args[4]) / scale;
										const rsb    = (+args[3] - +args[4]) / scale;
										const lead   =  +args[4] / scale;
										const top    = (y - rst) + lead;
										const bottom = (y - rsb) + lead;
										hotspot = {lead, top, bottom, x, y, meta};
										break;
									}
									...
									case "markend":
										if(hotspot){
											const {lead, meta} = hotspot;
											hotspots.push(hotspot = {
												x:      hotspot.x   - lead,
												y:      hotspot.top - lead,
												width:  (x - hotspot.x) + (lead * 2),
											...
											let thickness = 1;
											let colour = [0, 0, 0];
											hotspot.miscData = meta
												.replace(/\s*\/Color\s+\[\s*([^\]]+)\s*\]/g, (_, rgb) => {
													colour = rgb.split(/\s+/).map(c => +c * 0xFF);
													return "";
												})
												.replace(/\s*\/Border\s+\[([^\]]+(?:\[[^\]]+\])?)\s*\]/g, (_, values) => {
													values = values.split(/\s+/).filter(Boolean);
													cornerRadiusH = +values[0] || 0;
													cornerRadiusV = +values[1] || 0;
													thickness     = +values[2] || 0;
													return "";
												})
												.trim();
											hotspot = null;
