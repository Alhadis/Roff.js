...
const driver       = new DeviceDriver("/usr/local/share/groff/current/font/devpdf");
const {tokenise, tokenTypes} = require("./tokeniser.js");

module.exports = function(context, text){
	const pages = tokenisePages(text);
	const result = renderToCanvas(context, pages[0]);
	drawHotspots(context, result.marks);
};
...
function drawHotspots(context, marks){
	for(const mark of marks){
		const {border, x, y, width, height} = mark;
		if(border.thickness > 0){
			context.save();
			context.lineCap = "butt";
			context.lineJoin = "miter";
			context.lineWidth = border.thickness;
			context.setLineDash(border.dashGaps);
			context.strokeStyle = "rgb(" + border.colour.join(", ") + ")";
...
function renderToCanvas(context, tokens, zoom = 1.5){
	...
	const markStart = args => {
		...
		mark.meta = args.slice(5).join(" ")
			.replace(/\s*\/Border\s+\[([^\[\]]+)\s*(?:\[([^\]]+)\]\s*)?\]/, (_, values, dash) => {
				values = values.trim().split(/\s+/).map(n => +n);
				mark.border.cornerRadiusH = values[0];
				mark.border.cornerRadiusV = values[1];
				mark.border.thickness     = values[2];
				if(dash){
			...
			.replace(/\s*\/Color\s*\[([^\]]+)\]/, (_, values) => {
				mark.border.colour = values.trim().split(/\s+/).map(n => Math.round(+n * 0xFF));
				return "";
			})
			.trim();
	};
