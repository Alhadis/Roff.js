 ...
 *
 * @internal
 * @example splitOptions("-pvTpdf") == ["-p", "-v", "-T", "pdf"];
 * @param {Array<String[]>} argv
 * @param {String[]} [short=[]]
 * @param {String[]} [long=[]]
 * @return {String}
 */
export function splitOptions(argv, short = [], long = []){
	const patterns = [
		short[0] && new RegExp(`^-([${short[0]}]+${short[1] && `)([${short[1]}]?`})(\\S*)`),
		short[1] && new RegExp(`^(-[${short[1]}])(\\S*)`),
		long[1]  && new RegExp(`^(--${long})(=(\\S*))?(?=$|\\s)`),
	];
	const opts = [];
	while((argv = [...argv]).length){
		...
		if(patterns[0] && (match = arg.match(patterns[0]))){
			opts.push(...match[1].split("").map(s => "-" + s));
			match[2] && opts.push("-" + match[2]);
			match[3] && ((match[2] ? "" : "-") + opts.push(match[3]));
		}
		else if(patterns[1] && (match = arg.match(patterns[1]))) opts.push(...match.slice(1));
		else if(patterns[2] && (match = arg.match(patterns[2]))) opts.push(...match.slice(2));
		else opts.push(arg);
	}
