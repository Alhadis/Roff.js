...
const K_RATIO    = Symbol("TroffPageAspectRatio");
...
class TroffView{
	...
	constructor(attr = {}){
		...
		this.offsetLayer   = this.div(classNames.offsetLayer);
		...
		this.offsetLayer.appendChild(this.zoomLayer);
		this.root.appendChild(this.offsetLayer);
	...
	goToAnchor(dest){
		...
		if(targetObj){
			const index = targetObj.page.index - 1;
			const node = this.pagesList.children[index];
			if(!node) return;
