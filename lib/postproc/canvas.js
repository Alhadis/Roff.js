	...
	exec(cmd, data){
		...
		switch(cmd){
			...
			case tokenTypes.DEVICE_CTRL:
				const sizeCtrl = /^papersize=(\d+(?:\.\d+)?)([icpP]),(\d+(?:\.\d+)?)([icpP])/;
				if("X" === data[0] && sizeCtrl.test(data[1])){
					const units     = {i: 25.4, c: 10, p: 0.353, P: 4.2333};
					this.pageWidth  = +RegExp.$1 * units[RegExp.$2];
					this.pageHeight = +RegExp.$3 * units[RegExp.$4];
					if(this.page){
