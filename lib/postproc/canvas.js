...
class CanvasRenderer extends Postprocessor{
			...
			case tokenTypes.DEVICE_CTRL:
				...
				if(/\[\s*(\S.+)\s+pdfmark\b/.test(data)){
					...
					if(/^\/Dest\s+\/?(\S+)(.*)\s+\/DEST$/.test(data)){
						...
						this.anchors.set(RegExp.$1, {page: this.page, offset: +view[0]});
						return;
					}
	...
	get pageIndex(){
		return this.page
			? this.pages.indexOf(this.page)
			: -1;
	}
	
	set pageIndex(to){
		to = this.capPageIndex(to);
		this.page = this.pages[to] || this.page;
		this.renderPage(to);
	}
	...
	hotspotStart(args){
		this.hotspot = this.parsePDFHotspot(args);
	}
	...
	hotspotEnd(){
		if(null !== this.hotspot && !this.hotspotHold){
			...
			this.hotspot.draw(this.context);
			this.hotspot = null;
		}
	}
	...
	parsePDFHotspot(args){
		...
		hotspot.unparsedArgs = args.slice(4).join(" ")
			.replace(/\s*\/Border\s+\[([^\[\]]+)\s*(?:\[([^\]]+)\]\s*)?\]/, (_, values, dash) => {
				values = values.trim().split(/\s+/);
				hotspot.border.cornerRadiusH = +values[0] || 0;
				hotspot.border.cornerRadiusV = +values[1] || 0;
				hotspot.border.thickness     = +values[2] || 0;
				if(dash){
					dash = dash.trim().split(/\s+/).map(n => +n);
					hotspot.border.dashGaps = dash;
				}
				return "";
			})
			.replace(/\s*\/Color\s*\[([^\]]+)\]/, (_, values) => {
				values = values.trim().split(/\s+/).map(n => Math.round(+n * 0xFF) || 0);
				hotspot.border.colour = values;
				return "";
			})
			.replace(/\/Subtype\s+\/Link\s+(.+)/, (_, data) => {
				if(/^\/Dest\s+\/(\S+)\s*$/.test(data))
					hotspot.targetDest = RegExp.lastParen;
				else if(/^\s*\/Action\s+<<\s*\/Subtype\s+\/URI/.test(data))
					hotspot.targetURI = data.match(/\/URI\s+\((\S*)\)\s+>>/)[1];
				return "";
			})
			.trim();
		return hotspot;
	...
	print(text){
		...
		if(true === this.hotspotWrap && !this.hotspotHold){
			...
			lastHotspot && this.hotspotStart(lastHotspot.args);
			this.hotspotWrap = false;
		}
	}
