...
class CanvasRenderer extends Postprocessor{
	...
	process(source){
		...
		if(this.squashedText){
			this.page.push([tokenTypes.TEXT_NORMAL, this.squashedText]);
			this.squashedText = "";
		}
	}
	...
	exec(cmd, data){
		if(!cmd) return;
		if(this.squashedText && cmd !== tokenTypes.TEXT_NORMAL){
			if(cmd === tokenTypes.MOVE_ABS_H)
				return;
			this.page.push([tokenTypes.TEXT_NORMAL, this.squashedText]);
			this.squashedText = "";
		}
		switch(cmd){
			...
			case tokenTypes.DEVICE_CTRL:
				const sizeCtrl = /^papersize=(\d+(?:\.\d+)?)([icpPm]),(\d+(?:\.\d+)?)([icpPm])/;
				if("X" === data[0] && sizeCtrl.test(data[1])){
					const units = {
						i: n => parseInt(n * 72),
			...
			default:
				(this.fixKerning && cmd === tokenTypes.TEXT_NORMAL)
					? this.squashedText += data
	...
	renderPage(pageIndex, context){
		...
		for(const [type, data] of this.page){
			if(type === tokenTypes.DEVICE_CTRL)
				super.exec(type, data.replace(/^\s*X\S*\s*|\n\+/g, ""));
	...
	drawSpline(args){
		...
		for(let i = 0; i < numPoints; i += 2){
			...
			this.context.bezierCurveTo(
				x + ((args[i]   * tNum) / (2 * tDen)) / this.scale,
				y + ((args[i+1] * tNum) / (2 * tDen)) / this.scale,
				x + (args[i]   / 2 + (args[i+2] * (tDen - tNum)) / (2 * tDen)) / this.scale,
				y + (args[i+1] / 2 + (args[i+3] * (tDen - tNum)) / (2 * tDen)) / this.scale,
				x + nX,
				y + nY
			);
			this.x += nX;
	...
	fill(){
		this.context.fillColor = this.drawColour;
		this.context.fill();
		this.context.fillColor = this.colour;
	}
	...
	handleDrawingCommand(type, ...args){
		switch(type){
			...
			// Drawing colour
			case "F":
				this.drawColour = this.parseColour(this.lastCommand[1]);
				break;
				...
				switch(args[1]){
					...
					default:
						...
						if(/exec gsave currentpoint 2 copy translate .+ rotate neg exch neg exch translate/.test(code))
							this.setRotation(args[7]);
						
						// Unset rotation
						else if(/exec grestore/.test(code))
							this.setRotation(0);
						
						// Set stroke properties
						else if(/exec \d setline(cap|join)/.test(code))
							"cap" === RegExp.lastParen
								? (this.context.lineCap  = ["butt", "round", "square"][args[2]])
								: (this.context.lineJoin = ["miter", "round", "bevel"][args[2]]);
	...
	parseColour(data){
		let args = data.substr(1).trim().split(/\s+/);
	...
	parsePDFMark(args){
		const rst   = (+args[2] + +args[4]) / this.scale;
		const rsb   = (+args[3] - +args[4]) / this.scale;
		const lead  =  +args[4] / this.scale;
		const mark  = {
			args, lead,
			x: this.x - lead,
			y: this.y - rst,
	...
	updateTransformMatrix(){
		if(0 === this.slant && 0 === this.charHeight)
			this.tm = null;
		else{
			this.tm = [1,0,0,1,0,0];
