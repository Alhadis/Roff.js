import TTYRenderer from "./renderer.mjs";
 ...
 *
 * @event   link
 * @param   {HTMLAnchorElement} element - The <a> tag the user clicked on
 ...
 */
export default class TTYViewer {
	...
	/**
	 * Synchronously invoke each listener registered for an event.
	 *
	 * @param {String|Symbol} eventName
	 * @param {...*} [args] - Optional arguments to pass to listener
	 * @internal
	 */
	emit(eventName, ...args){
	...
	/**
	 * Register a function to be run each time an event is triggered.
	 *
	...
	on(eventName, handler){
		...
		else if(-1 === handlers.indexOf(handler))
