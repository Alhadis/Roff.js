...
const ManPageReference = require("../../system/man/page-reference.js");
...
class TTYViewer{
	...
	goToAnchor(dest){
		if(dest instanceof HTMLElement)
			...
			dest = ManPageReference.slugify(dest);
	...
	handleClick(event){
		...
		const parts = ManPageReference.fromURL(link.href);
		(null === parts)
	 ...
	 * Conversion is handled by {@link ManPageReference.slugify},
	...
	remapLandmarks(selector = "b", preserveCase = false){
		...
		for(const tag of this.element.querySelectorAll(selector)){
			...
			if(!preserveCase)
				title = ManPageReference.slugify(title);
