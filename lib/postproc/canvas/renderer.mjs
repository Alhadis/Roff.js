import Phototypesetter from "../phototypesetter.mjs";
...
export default class TroffCanvasRenderer extends Postprocessor{
	...
	process(source){
		// Ancient C/A/T instructions
		if(0x40 === source[0] && 0xEF === source[1])
			source = (new Phototypesetter("ps")).process(source);
	...
	exec(cmd, data){
		...
		if(null !== this.buffer && !isSquashableCmd){
			if(cmd === tokenTypes.INFORM_EOL)
				...
				const squashedText = this.buffer
					.map(cmd => cmd[0] === tokenTypes.TEXT_NORMAL ? cmd[1] : "")
					.join("");
