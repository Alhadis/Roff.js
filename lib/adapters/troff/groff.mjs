...
export default class GroffAdapter {
	...
	async format(input, device, options = {}){
		...
		const includePaths = Array.from(options.includePaths || []).filter(Boolean);
		if(options.expandLinks){
			const {resolve, dirname} = await import("path");
			options.inputFile     && includePaths.push(resolve(dirname(options.inputFile)));
			options.macroFilePath && includePaths.push(options.macroFilePath);
		}
	 ...
	 * @param {Object} opts - Options object, modified in-place.
	 * @internal
	 */
	resolvePreprocessors(opts){
		let pipe = opts.preprocessors || [];
