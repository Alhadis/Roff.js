 ...
 * @class
 */
export default class GroffAdapter extends Adapter {
	...
	/**
	 * Shell out to Groff to obtain info that's needed before instance can be used.
	 * @return {Promise}
	 * @internal
	 */
	async resolve(){
		return super.resolve(async () => {
			const deviceList = "dvi html lbp lj4 ps pdf ascii cp1047 latin1 utf8".split(" ");
			await this.resolveVersion();
			await this.resolveDeviceSupport(deviceList);
			await this.resolveMacros();
		});
	}
	
	
	/**
	 * Determine availability of postprocessors.
	 * @param {String[]} [list]
	 * @internal
	 */
	async resolveDeviceSupport(list){
		...
		this.devices[name] = await exec(this.path, ["-ZT", name], "Hello, world\n")
			...
			.catch(() => false);
		return list.length && this.resolveDeviceSupport(list);
	}
	
	
	/**
	 * Determine what macros are available.
	 * @internal
	 */
	async resolveMacros(){
