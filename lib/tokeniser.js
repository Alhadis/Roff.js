		...
		for(let i = 0; i < length; ++i){
			...
			switch(type){
				...
				case DD0:{
					...
					switch(cmd){
						case "l":
							const x = (+args[0] || 0) / res[1];
							const y = (+args[1] || 0) / res[2];
							if(0 === y && x > 0){
								output += "─".repeat(x);
							}
							break;
				...
				case UU0:
					column += data.length;
					output += textTag[0] + data.replace(/<|>|&/g, c => `&#${c.charCodeAt(0)};`) + textTag[1];
					break;
				
				// Print character
				case Cc0:
					offsetH = 1;
					output += textTag[0] + data + textTag[1];
					break;
				
				// Print character by index
				case NN0:
					offsetH = 1;
					output += textTag[0] + String.fromCharCode(+data) + textTag[1];
					break;
				
				// Print character by name
				case CC0:
					offsetH = 1;
					output += textTag[0] + chars[data] + textTag[1];
					break;
